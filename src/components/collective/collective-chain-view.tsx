...
export function CollectiveChainView({ detail, isLoading, fixtureName }: CollectiveChainViewProps) {
  ...
  const resolvedView = detail?.view ?? null;
  const preparedEffectNode = resolvedView?.nodes.find((node) => node.stage === "preparedEffect" && node.isPresent) ?? null;
  const preparedEffect = useQuery({
    queryKey: preparedEffectNode?.recordId
      ? collectiveObservabilityQueryKeys.preparedEffects.detail(preparedEffectNode.recordId)
      : ["collective", "prepared-effects", "detail", "absent"] as const,
    queryFn: () => createPreparedEffectRepository().detail(preparedEffectNode!.recordId!),
