...
import { getCurrentStepIndex, onboardingSteps } from "@/lib/onboarding/state-machine";
...
export function OnboardingLayout({ children }: { children: React.ReactNode }) {
  ...
  const currentIndex = Math.max(0, getCurrentStepIndex(currentStep));
  const progressValue = currentStep === "COMPLETE" ? onboardingSteps.length : currentIndex + 1;
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,rgba(102,252,241,0.14),transparent_28%),radial-gradient(circle_at_bottom_right,rgba(255,176,0,0.08),transparent_24%),#071116] text-[#F4F7F8]">
      ...
      <div className="relative flex min-h-screen flex-col">
          ...
          <div className="mx-auto flex w-full max-w-6xl items-center justify-between gap-6 px-6 py-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="font-mono text-[11px] uppercase tracking-[0.22em] text-[#7EE8E0]/80">Guided activation</div>
              </div>
            </div>
            <div className="hidden min-w-[240px] flex-1 items-center justify-center md:flex">
              <div className="w-full max-w-sm space-y-2">
                <div className="flex items-center justify-between font-mono text-[11px] uppercase tracking-[0.22em] text-white/70">
                  <span>Step {progressValue} of {onboardingSteps.length}</span>
                  <span>{formatStepLabel(currentStep)}</span>
                </div>
            ...
            <div className="rounded-full border border-white/10 bg-white/5 px-4 py-2 font-mono text-xs text-white/70">Account ready</div>
          </div>
        </header>
        ...
        <main className="relative flex flex-1 items-center justify-center px-4 py-10 sm:px-6 lg:px-8">
          ...
          <div
            className={cn(
              "relative w-full max-w-4xl overflow-hidden rounded-[28px] border border-white/10 bg-[linear-gradient(180deg,rgba(7,20,27,0.96),rgba(3,9,13,0.94))] shadow-[0_24px_120px_rgba(0,0,0,0.45)]",
